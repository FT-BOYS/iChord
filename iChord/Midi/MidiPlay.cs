...
namespace iChord
{
    class MidiPlay
    {
        ...
        {
            playThread_right(device, note, 0, 0, 127);
        ...
        public static void playMidi(MidiDevice device, string note, int n0, string chord, int n1, string music, int n2)
        {
            ...
            playThread_left(device, chord, n1, 1, 100);
            playThread_left(device, music, n2, 2, 100);
        }
        /// <summary>
        /// 播放一小节函数，供NewPlay函数调用（四轨道重载）
        /// 默认主旋律音量127=max 和弦轨音量100
        ...
        /// <param name="和弦三轨"></param>
        /// <param name="音色三"></param>
        public static void BrandNew(MidiDevice Device, string inf1, int n1, string inf2, int n2, string inf3, int n3, string inf4, int n4)
        {
            MutilThread engine = new MutilThread();
            engine.setdevice(Device);
            engine.setinf1(inf1);
